...
`r df2[which.max(df2$MeanSteps),]$interval` and that average number of steps is 
`r round(max(df2$MeanSteps), digits=1)`


## Imputing missing values
There are a number of intervals for which we do not have the number of steps recorded. There are `r  sum(!complete.cases(activity))` such intervals missing values.
